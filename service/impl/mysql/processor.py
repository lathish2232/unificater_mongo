...
def process_mysql_query(db_name, host, port, user, password, query, database):
    try:
        con = get_db_connection(db_name, host, port, user, password, database)
        rows = pd.read_sql(query, con)
    except Exception as ex:
...
def get_meta_info(meta_data, host, port, user, password, database=None):
    try:
        ...
        con = get_db_connection('mySql', host, port, user, password, database)
        cur = con.cursor()
...
def get_mysql_table_describe(db_name, host, port, user, password, query, database, schema):
    try:
        con = get_db_connection(db_name, host, port, user, password, database)
...
def extract_mysql_data(db_name, host, port, user, password, query, database, schema=None):
    try:
        con = get_db_connection(db_name, host, port, user, password, database)
        cur = con.cursor()
        if schema:
