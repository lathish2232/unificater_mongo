...
def _factorize_keys(
    ...
    # Commented for Unificater to handle null values
    # if lany or rany:
    #     if lany:
    ...
    # The Unificater changes starts from here >>>>>> 
    if lany or rany:
        if lany:
            lna_cnt=np.count_nonzero(lany)
            litr = (x+count for x in range(lna_cnt))
            np.putmask(llab, lmask, np.fromiter(litr, int))
            count += lna_cnt
        if rany:
            rna_cnt=np.count_nonzero(rany)
            ritr = (x+count for x in range(rna_cnt))
            np.putmask(rlab, rmask, np.fromiter(ritr, int))
            count += rna_cnt
    # The Unificater changes ends here <<<<<< 
        count += 1
